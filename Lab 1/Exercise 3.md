...
- Take a look at these code and description:
  ...
  - I found that the first instruction `lgdtw 0x7c64` just loads the Global Descriptor Table (GDT) with the base address 0x7c64, which is the segment that define memory segments.
  - And then the next 3 instruction modify the `%cr0` register, which containt control flag and then set the Protection Enable flag to 1 (enable) by `or` with 1.
  - After 3 instruction above, the program are now switched to Protection mode (32-bit mode) and be able to execute 32-bit code segment at 0x7c32.
